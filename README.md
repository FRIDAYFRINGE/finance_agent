...
The repository includes a comprehensive 4-tier evaluation runner that maps directly to reviewer benchmarks:

```bash
# 1. Run complete offline invariant test suite (0 API credits, ~3s):
python scripts/test_e2e_evaluation.py

# 2. Run targeted live LLM benchmark evaluation:
python scripts/test_e2e_evaluation.py --live
```

### Verified Test Results (Latest Repository Verification Run):
* **Offline Deterministic Invariant Suites (Tiers 1–3)**: **11/11 Passed** (Configs, SQLite relational integrity, NVDA headcount contract, TSLA out-of-scope contract, FastAPI endpoints, cross-interface parity; 166/166 assertions across all modular test scripts).
* **Targeted Live LLM Benchmarks (Tier 4)**: **8/8 Passed** against `z-ai/glm-5.3-flash` via OpenRouter (all reviewer evaluation scenarios verified).
* **Cross-Persona Divergence**: Jaccard similarity between personas on the identical Tech prompt was **0.235–0.243** (well below the `< 0.65` divergence requirement, demonstrating distinct analytical framing).
* **Grounding Stress Test**: Accurately retrieves **29,600 / +13.0% YoY** for NVDA directly from SQLite via MCP.
* **Honest Scope Awareness**: Queries on unmodeled tickers (`TSLA`) return honest refusals with `companies_referenced: []` and 0 phantom citations.

---

## 9. Known Data Caveats & Limitations

1. **Temporal Snapshot & Sourcing Anchor**: Historical financials cover audited **FY2023–FY2024**. Valuation records are maintained as a curated **Q4 2024 snapshot**. The attached Yahoo Finance quote URLs are live endpoints and do not independently provide point-in-time archival evidence for every 2024-12-31 value.
2. **Derived Metric Definitions**: Some fields such as FCF, EBITDA, ROE, and certain leverage ratios may be derived or definition-sensitive across different reporting conventions; the project uses standardized curated dataset definitions and preserves the underlying primary source references.
3. **Capital Intensity Proxy**: Detailed 5-year LBO models approximate capital reinvestment via reported `free_cash_flow` and `FCF / EBITDA` conversion rather than unmodeled working capital schedules.
4. **Sector Aggregates**: Sector averages are computed dynamically across database records via SQL and attributed as derived multi-company aggregates (`source_url: ""`).

---

## 10. One Thing I'd Improve with More Time

**Automated Multi-Period Line-Item Filing Diffing**:  
With more time, I would implement an MCP tool that parses consecutive 10-K filing XBRL disclosures to dynamically compute multi-period margin waterfall bridges (e.g. Price vs. Volume vs. Mix variance). This would allow the Equity and PE analysts to programmatically walk through exact cost-of-goods inflation and SG&A leverage bridges without relying on LLM arithmetic over raw numbers.
